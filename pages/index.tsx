...
const Page = ({ content, articles, repositories }: IPageHomeProps) => {
  return (
    <main>
      ...
      <section>
        <h2>
          {content.section_repositories_title}
        </h2>
        <ul>
          {repositories.map((repo, i) => <li data-testid='repository' key={i}>
            {repo.name}
            <p>
              {repo.description}
            </p>
  ...
  try {
    ...
    return {
      props: {
        ...
        repositories: repositories.map((repository: IGithubRepository) => ({ // Should be another type here that would fully describe a Github repository but oh well
